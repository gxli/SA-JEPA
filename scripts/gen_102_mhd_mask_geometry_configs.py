#!/usr/bin/env python3
"""Generate gen_102 MHD/C12 mask-geometry sweep configs with gen_97 network behavior."""
...
def main() -> None:
    ...
    for stale_path in glob(os.path.join(OUT_DIR, "gen_102_*.json")):
    ...
    cfg = _pyramid_base()
    paths.append(_write(f"gen_102_mhd_run_{run:03d}_pyramid_ms1p1", cfg))
    run += 1
