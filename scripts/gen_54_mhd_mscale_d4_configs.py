#!/usr/bin/env python3
"""Generate gen_54 MHD configs (mask_scale sweep)."""
...
BASE_MODEL = {
    "mode": "pyramid",
    "blur_mode": "cdd",
    "model_key": "cdd_scaleaware_convnext-pyramid-scaleaware-d4",
    "sigmas": [2, 4, 8, 16],
    "latent_channels": 32,
    "encoder_width": 64,
    "encoder_depth": 4,
...
def main() -> None:
    ...
    for mscale in MASK_SCALES:
        ...
        m["mask_size_scaling"] = float(mscale)
        name = (
            "gen_54_run_1_mhd_cdd_scaleaware_convnext-pyramid-scaleaware-d4_"
            f"afrac_1p0_mscale_{_fmt(mscale)}_mbox_00"
        )
        write_config(name, m)
        count += 1
