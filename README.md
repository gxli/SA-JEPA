...
Optional GPU UMAP acceleration uses RAPIDS/cuML and is not installed by
default. On Linux x86_64 CUDA 12 environments, install the extra with the
...
# 4. Generate/open dashboards from saved inference/embedding artifacts
dashboard_html = model.open_dashboard()                # → outputs/my_run/dashboard.html
umap_html = model.open_interactive_umap("predict")     # → outputs/my_run/results/interactive_umap_predict.html
# Other branches: "context", "target", and "masked_predict" when present.
...
dashboard_html = model.open_dashboard()
umap_html = model.open_interactive_umap("predict")
...
For fields that do not fit in memory, first reduce `train.batch_size` and use
`train.gradient_accumulation_steps` to preserve the effective batch size. If the
field itself is too large, train on spatial crops with `data.crop_size` and
`data.crop_mode` (for example `400` and `"random"`). See
`configs/examples/masked_crop_blueprint.yaml`.

Scale-aware CDD encoders compute CDD once up front and reuse the session
`cdd_cache/`; they do not recompute CDD every batch. If the cache would exceed
memory, reduce the number or size of training fields, lower the crop size, or
raise `data.cdd_precompute_max_gb` deliberately.

Target-validity masks are separate from JEPA context masks. To restrict where
target centers may be sampled, provide `data.target_mask: path/to/mask.npy`
with values `>0` marked valid, or set `data.target_threshold: 1e-6` to build a
valid-target map from the raw data. The saved `target_mask_map` dashboard
artifact shows the JEPA masking footprint, not the user-provided validity map.

Post-training dashboard inference runs on the uncropped field when possible and
uses tiled dense inference internally for large 2D maps. For inference-only runs
on large fields, call `model.infer_npy(..., crop_size=..., crop_mode="tile")`.
...
For fields larger than $\sim 512^2$ px, GPU memory often becomes the limiting
...
- Training: set `data.crop_size` and `data.crop_mode` in YAML.
  ...
  Use `data.target_mask` or `data.target_threshold` to restrict valid target
