...
umap_html = model.open_interactive_umap() # optional interactive UMAP view
...
For 3D sessions, volumetric UMAP uses the full valid inferred slice/slab/volume
extent, capped only by `train.umap.volumetric_max_points` (default `100000`).
There is no default fraction sampling.

## ⚙️ Hyperparameter Knobs

Config files should either declare `base_config` or be run through the training
loader, which merges the project base before applying overrides. The exhaustive
breakdown of the main parameters is in
